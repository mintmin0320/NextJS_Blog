...
```
/* node_modules > ErrorBoundary 라이브러리 */
...
node_modules에서 ErrorBoundary 파일을 살펴보면, 
- 보일러플레이트 코드와 동일하게 getDerivedStateFromError 생명 주기 메서드를 사용해 에러가 발생한 컴포넌트 상태를 업데이트 하고 있다. (폴백 UI가 보이도록 상태를 업데이트)
- componentDidCatch 메서드의 모습도 보인다.
